...
The Base Network Zone Object represents the Parent Class from which concrete Network Zone types(IP and Dynamic) inherit.
...
| system           | Indicates if this is a system network zone. For admin-created zones, this is always `false`                                                                 | boolean                                        | No             | N/A   |
...
While no individual attribute is required, at least one of the three must be defined.

The follow attributes are defined by Dynamic Zone objects:

| Field Name     | Description                                                                                 | Data Type                                     | Required        | Max Length    |
| :------------- | :------------------------------------------------------------------------------------------ | :-------------------------------------------- | :-------------- | :------------ |
| proxyType       | One of the following: `""`(For Unspecified), `Any`, `Tor`, `NotTorAnonymizer`                                            | String   | No              | N/A |
| locations        | IP addresses (range or CIDR form) allowed to forward request from gateway addresses above   | Array of [Location Objects](#location-object)   | No              | 150 (entries) |
| asns        | Format of each array value: a String representation of an ASN numeric value   | Array of Strings   | No              | 150 (entries) |
